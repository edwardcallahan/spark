...
function build_spark {
  ...
  $MAKE_DISTRO_PATH --name $SPARK_TGZ_NAME --tgz  -Pnetlib-lgpl -Psparkr -Phive -Phive-thriftserver -Pmesos -Phadoop-"${HADOOP_VERSION%.*}" $SCALA_VERSION_PARAM
}
...
function start_docker_cluster {
  ...
  SPARK_FILE="$(ls ${SPARK_BUILD_HOME}/*${SPARK_TGZ_NAME}.tgz)"
...
function parse_args {
  ...
  while :; do
    case $1 in
      ...
      ;;
      --spark-tgz-name)       # Takes an option argument, ensuring it has been specified.
      if [ -n "$2" ]; then
        export SPARK_TGZ_NAME=$2
        shift 2
        continue
      else
        exitWithMsg '"--spark-tgz-name" requires a non-empty option argument.\n'
      fi
      ;;
